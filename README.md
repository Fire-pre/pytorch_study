...
#2026 3 4
很久都没更新啦，放寒假就懈怠了，中间也有学习但是没有更新上来新学了怎么脸上autodl的cpu现在跑起来快很多啦，电脑也不烫了。还是在simvp看来没有按时完成任务呢，不知道什么时候可以结束这篇论文哈哈哈哈现在已经在跑指标了，实际上很多还是要靠大模型的指导。

#2026 3 9
已经跑出图啦，convlstm实验出了点问题，预测的图特别模糊，但是simvp的看起来10帧预测10帧还是比较准的，就这两个还对比了一下，接下来做一下消融实验看看吧

#2026 3 23 中间拿ADS-B数据试了一下，很不幸结果不好哈哈哈，这段时间偷懒没写，今天再写点吧，看了一篇新论文因为SimVP和我的数据并步match只能转换思路了，当前的深度学习模型设计的是处理深度，长序列输入因此在处理低频离散的遥感环境效果并不好，基于空间的遥感补充了地面监视系统，特别的是海洋和偏远地区，包含长时间的重访间隔、对云层敏感和不连续的航迹观测等固有限制所以用传统方法很难处理。在没有实时观察更新或可靠先验的情况下，递归方法容易出现累积误差漂移，而大多数现有的深度学习模型缺乏强制保持轨迹一致性的结构机制。好啦这是今天的学习内容啦。

#2026 3 24 HiFormer模型使用CNN层来捕捉局部运动特征，LSTM进行中值域时空建模，Transformer编码器来捕捉长时依赖。这些组件通过一个时间注意力过编码器输出机制动态协调，该机制自适应地重新加权历史观察，以关注再稀疏采样下最具信息量的时间步。公开的基于空间的航空器轨迹数据缺乏数据，所以这篇文章通过数值模拟生成了12000条数据，包含了三种类型的航空器性质和四种规范的运动形态（匀速直线、匀加速运动、匀速转弯、匀加速转弯）。为了数据的多样性在最后使用1000条ADS-B数据测验(flight ID,timestamp, longitude, and speed)，一秒一帧，输入20帧预测20帧。网格划分为2048*2048，时间范围为50s，分辨率为600.模型在一次前向传播的过程中输出整个未来的航迹序列，减轻了循环神经网络的误差累计。合成数据包含8个参数：X-position,Y-position,velocity along the X-axis,vlocity along the Y-axis,acceleration along the X-axis,acceleration along the Y-axis,yaw angle,and angular velocity.ADS-B数据是稀疏、低频、离散的。

#2026 3 25 今天看了论文的一些参数设置，以及数据预处理，去除重复条目，分割滑动窗口，进行缩放分区。输入窗口时间不为W，输出为H，如果数据量小于W+H则舍弃。用最大最小化规则对训练集进行归一化。选择显著较少的帧可能会遗漏重要的运动线索，而更长的片段则会增加冗余和延迟，而没有成比例的准确率提升。T=20很好的平衡了这个问题。

#2026 3 26 今天继续看了模型的参数设置研究了大致维度变化，看了对比试验。
